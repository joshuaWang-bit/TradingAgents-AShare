...
import { getReportDisplayTitle, getReportSecurityName } from '@/utils/reportText'
import { buildWechatShareHtmlDocument } from '@/utils/wechatReportImage'
...
function exportReport(report: ReportDetail) {
    ...
    const text = [
        ...
        '',
        `- 股票代码：${report.symbol}`,
        getReportSecurityName(report) ? `- 股票名称：${getReportSecurityName(report)}` : '',
...
export default function Reports() {
    ...
    const [exportingWechatHtml, setExportingWechatHtml] = useState(false)
    ...
    const handleExportWechatHtml = useCallback(async () => {
        if (!selectedReport) return

        setExportingWechatHtml(true)
        try {
            const html = buildWechatShareHtmlDocument(selectedReport)
            const blob = new Blob([html], { type: 'text/html;charset=utf-8' })
            const url = URL.createObjectURL(blob)
            const anchor = document.createElement('a')
            anchor.href = url
            anchor.download = `analysis-wechat-${selectedReport.symbol}-${selectedReport.trade_date}.html`
    ...
    if (selectedReport) {
        ...
        const reportDisplayTitle = getReportDisplayTitle(selectedReport) || selectedReport.symbol
        const reportSecurityName = getReportSecurityName(selectedReport)
            ...
            <div className="space-y-6">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="text-xl font-bold text-slate-900 dark:text-slate-100">
                        {reportDisplayTitle} 分析报告
                    </h1>
                    <button
                        onClick={handleExportWechatHtml}
                        disabled={exportingWechatHtml}
                        className="ml-auto flex items-center gap-1.5 px-3 py-1.5 text-sm rounded-lg bg-amber-50 text-amber-700 hover:bg-amber-100 disabled:opacity-60 disabled:cursor-not-allowed transition-colors border border-amber-200"
                    >
                        {exportingWechatHtml ? <Loader2 className="w-4 h-4 animate-spin" /> : <Download className="w-4 h-4" />}
                        {exportingWechatHtml ? '正在导出 HTML…' : '导出微信分享 HTML'}
                    </button>
                    <button
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 items-start">
                    {selectedReport.status === 'completed' ? (
                        <DecisionCard
                            ...
                            name={reportSecurityName ?? undefined}
                            decision={action}
