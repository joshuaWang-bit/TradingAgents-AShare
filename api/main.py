...
def _cors_allow_origin_regex() -> str | None:
    ...
    if raw:
        return raw
    # Local desktop usage may mix localhost and 127.0.0.1, which turns the API
    # call into a cross-origin request even though everything stays on the same
    # machine. Allow loopback origins on any port by default.
...
async def _run_job(
    ...
    try:
        ...
        if request.query:
            ...
            async def _process_horizon(horizon: str):
                ...
                try:
                    ...
                    horizon_errors[horizon] = f"{type(e).__name__}: {e}"
            ...
            for i, r in enumerate(results):
                if isinstance(r, Exception):
                    ...
                    _log(f"Horizon '{horizon}' failed: {r}")

            if len(failed_horizons) == len(request.horizons):
                raise RuntimeError(
                    "All requested horizons failed: "
                    + "; ".join(
                        f"{h} -> {horizon_errors.get(h, 'unknown error')}"
                        for h in failed_horizons
                    )
                )
            ...
            primary_has_content = any(primary_r.get(key) for key in report_keys)
            if not primary_has_content:
                details = "; ".join(
                    f"{h} -> {horizon_errors.get(h, 'empty result')}"
                    for h in request.horizons
                )
                raise RuntimeError(
                    "Analysis produced no report content. "
                    f"Upstream model may be unavailable. Details: {details}"
                )
        ...
        single_has_content = any(result.get(key) for key in report_keys)

        if stream_events and stream_error and not result.get("final_trade_decision"):
            raise RuntimeError(
                "Analysis ended before a final trade decision was produced. "
                f"Upstream model may be unavailable. Details: {stream_error}"
            )

        if not single_has_content:
            raise RuntimeError("Analysis produced no report content.")
...
def _resolve_symbol_locally(text: str) -> tuple[Optional[str], Optional[str]]:
    """Best-effort local symbol resolution before asking the LLM.

    This keeps common A-share queries like "分析一下宁德时代今天走势" from failing
    just because the intent-extraction model did not return the expected JSON.
    """
    symbol, date = _extract_symbol_and_date(text)
    if symbol:
        return symbol, date

    local_code = _search_cn_stock_by_name(text.strip())
    if local_code:
...
async def _ai_extract_symbol_and_date_streaming(
    ...
    local_symbol, local_date = await asyncio.to_thread(_resolve_symbol_locally, text)
...
"""
    ...
    if not llm_name:
        if local_symbol:
            _log(f"[StockExtract] local fallback match: '{text[:40]}' → {local_symbol}")
            return local_symbol, local_date or today, llm_horizons, llm_focus_areas, llm_specific_questions, llm_user_context
...
def _ai_extract_symbol_and_date(
    ...
    today = datetime.now().strftime("%Y-%m-%d")
    local_symbol, local_date = _resolve_symbol_locally(text)
